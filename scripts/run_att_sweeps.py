...
sweep_config = {
    ...
    "parameters": {
        ...
        "use_attention": {"value": True}  # Fixed to True for attention experiments
    }
}

def make_run_name(config):
    attention_suffix = "_attn" if config.get('use_attention', False) else ""
    return (
        ...
        f"lr_{config['lr']}_"
        f"tfr_{config['teacher_forcing_ratio']}_"
...
def wandb_train(config=None):
    with wandb.init(config=config):
        ...
        model = build_model(            
            ...
            use_attention=config.use_attention,  # Added attention parameter
            device=device
        )
